...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="group border border-gray-100 rounded-[2.5rem] p-5 hover:shadow-2xl transition duration-500 bg-white flex flex-col h-full">
      ...
      <div className="flex-grow">
        <Link to={`/product/${product.id}`}>
          <h3 className="font-bold text-gray-800 text-lg mb-1 truncate hover:text-blue-600 transition">{product.name}</h3>
        </Link>
            ...
            <div className="flex flex-wrap gap-2">
              {sizeOptions.map(size => (
                <button
                  ...
                  className={`w-10 h-10 rounded-xl text-xs font-bold transition flex items-center justify-center border-2 ${selectedSize === size ? 'bg-black text-white border-black shadow-lg scale-110' : 'bg-gray-50 text-gray-500 border-transparent hover:border-gray-200'}`}
                >
                  {size}
                </button>
              ))}
      ...
      <button 
        ...
        className="w-full bg-gray-900 text-white py-4 rounded-2xl font-black text-sm hover:bg-blue-600 transition shadow-xl active:scale-95 transform mt-auto"
      >
        THÊM VÀO GIỎ
      </button>
    </div>
